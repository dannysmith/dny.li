...
async function createSessionRequest(url: string, options: RequestInit = {}) {
  ...
  const loginResponse = await worker.fetch(loginRequest, testEnv, {} as ExecutionContext)
...
describe('Admin UI', () => {
  describe('Authentication', () => {
    it('should redirect unauthenticated users to login', async () => {
      ...
      expect(response.headers.get('Location')).toBe('http://localhost:8787/admin/login')
    })
    ...
    it('should login with correct password', async () => {
      ...
      expect(response.status).toBe(302)
      expect(response.headers.get('Location')).toBe('http://localhost:8787/admin')
      expect(response.headers.get('Set-Cookie')).toContain('url_shortener_session=')
    })
  ...
  describe('Main Admin Page', () => {
    ...
    it('should display success message from query params', async () => {
      const request = await createSessionRequest('http://localhost:8787/admin?success=Test%20success%20message')
      const response = await worker.fetch(request, testEnv, {} as ExecutionContext)
      const html = await response.text()
    ...
    it('should display error message from query params', async () => {
      const request = await createSessionRequest('http://localhost:8787/admin?error=Test%20error%20message')
      const response = await worker.fetch(request, testEnv, {} as ExecutionContext)
      const html = await response.text()
  ...
  describe('Form Submissions', () => {
    it('should create URL and redirect back to admin', async () => {
      ...
      const request = await createSessionRequest('http://localhost:8787/admin/create', {
        method: 'POST',
        body: formData
      })
    ...
    it('should update URL and redirect back to admin', async () => {
      ...
      const request = await createSessionRequest('http://localhost:8787/admin/update/test-slug', {
        method: 'POST',
        body: formData
      })
    ...
    it('should delete URL and redirect back to admin', async () => {
      ...
      const request = await createSessionRequest('http://localhost:8787/admin/delete/test-slug', {
        method: 'POST'
      })
  ...
  describe('Edit Form', () => {
    it('should display edit form for existing URL', async () => {
      const testURL = createTestURL('test-slug', 'https://example.com/edit-test')
      await testEnv.URLS_KV.put('urls:test-slug', JSON.stringify(testURL))
      
      const request = await createSessionRequest('http://localhost:8787/admin/edit/test-slug')
      const response = await worker.fetch(request, testEnv, {} as ExecutionContext)
      const html = await response.text()
    ...
    it('should return 404 for non-existent URL', async () => {
      const request = await createSessionRequest('http://localhost:8787/admin/edit/non-existent')
      const response = await worker.fetch(request, testEnv, {} as ExecutionContext)
  ...
  describe('Logout', () => {
    it('should logout and redirect to login page', async () => {
      const request = await createSessionRequest('http://localhost:8787/admin/logout', {
        method: 'POST'
      })
